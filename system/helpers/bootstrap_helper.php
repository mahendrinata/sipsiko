...
function bootstrap_breadcrumb($controller = NULL, $action = NULL, $dir = 'admin') {
  $output = bootstrap_tag_open('ul', array('class' => 'breadcrumb'));
  $output .= bootstrap_tag('li', bootstrap_tag('a', '<i class="icon-home"></i> SIPD Jember', array('href' => base_url() . bootstrap_index_page() . $dir . '/dashboard/index')) . ' ' . bootstrap_tag('span', '/', array('class' => 'divider')));
  if ($action != 'index') {
    $output .= bootstrap_tag('li', bootstrap_tag('a', bootstrap_breadcrumb_name($controller), array('href' => base_url() . bootstrap_index_page() . $dir . '/' . $controller . '/index')) . ' ' . bootstrap_tag('span', '/', array('class' => 'divider')));
...
function bootstrap_table_nav_dropdown($title = NULL, $controller = NULL, $actions = array(), $dir = 'admin') {
  ...
  if (isset($actions['search'])) {
    $search .= bootstrap_tag_open('div', array('class' => 'navbar-form form-search'));
    ...
    $search .= form_input('search', NULL, 'placeholder="Masukkan Kata Kunci" class="search-query"');
...
function bootstrap_table_action_dropdown($controller = NULL, $actions = array(), $dir = 'admin') {
  ...
  $output .= bootstrap_tag_open('ul', array('class' => 'dropdown-menu'));
  foreach ($actions as $key => $value) {
    if ($key == 'delete' || $key == 'delete_child') {
      $output .= bootstrap_tag('li', bootstrap_tag('a', bootstrap_tag('i', '', array('class' => $icon_action[$key])) . ' ' . $value['name'], array('onclick' => 'return confirm(\'Apakah anda akan menghapus data ini?\');', 'href' => base_url() . bootstrap_index_page() . $dir . '/' . $controller . '/' . $value['action'])));
    } else {
      $output .= bootstrap_tag('li', bootstrap_tag('a', bootstrap_tag('i', '', array('class' => $icon_action[$key])) . ' ' . $value['name'], array('href' => base_url() . bootstrap_index_page() . $dir . '/' . $controller . '/' . $value['action'])));
    }
  }
